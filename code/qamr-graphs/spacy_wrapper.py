...
class ParsedQuestion:
    ...
    def get_right_chunks(self):
        """
        Get all chunks on the right side of the predicate
        Format: list of (rel, chunk)
        """
        return [(rel, chunk)
                for (rel, chunk) in self.children
                if chunk.side == Sides.RIGHT]

    def get_left_chunks(self):
        """
        Get all chunks on the left side of the predicate
        Format: list of (rel, chunk)
        """
        return [(rel, chunk)
                for (rel, chunk) in self.children
                if chunk.side == Sides.LEFT]

    def get_prep(self, collapsed_rel):
        """
        Get the preposition from a possibly collapsed relation
        """
        # If this is a collapsed relation, return its second part
        if collapsed_rel.startswith("prep_"):
            return collapsed_rel.split("prep_")[1]

        # Else - return None
        return None

    def propogate_collapse(self):
        """
        Decide if there's a need to move the preposition from
        the arguments to the predicate.
        If not, move each preposition to its respective argument.
        """
        # Check if all right-side arguments agree on preposition
        right_chunks = self.get_right_chunks()
        right_rels = list(set(map(itemgetter(0),
                                  right_chunks)))
        if len(right_rels) == 1:
            # (1) Yes - add the preposition to the head
            prep = self.get_prep(right_rels[0])
            if prep is not None:
                self.root.toks.append(prep)
        else:
            # (2) No - add the preposition to the argument
            for (rel, chunk) in right_chunks:
                prep = self.get_prep(rel)
                if prep is not None:
                    rel.toks.append(prep)
